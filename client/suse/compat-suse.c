...
static ni_suse_ifcfg_t *		ni_suse_ifcfg_add_l2_port(ni_suse_ifcfg_array_t *,
							...
							ni_bool_t);
static ni_suse_ifcfg_t *		ni_suse_ifcfg_add_ovs_system(ni_suse_ifcfg_array_t *,
							ni_suse_ifcfg_t *);
...
static ni_suse_ifcfg_t *		ni_suse_ifcfg_find_by_ifname(ni_suse_ifcfg_array_t *,
							const char *);
static ni_suse_ifcfg_t *		ni_suse_ifcfg_find_by_iftype(ni_suse_ifcfg_array_t *,
							ni_iftype_t);
...
try_add_ovs_bridge_port(ni_suse_ifcfg_array_t *ifcfgs, ni_suse_ifcfg_t *ifcfg,
		const char *port, const char *variable, const char *suffix)
{
	ni_netdev_port_config_t *pconf;
	ni_suse_ifcfg_t *pcfg;
	ni_suse_ifcfg_t *scfg;
	ni_netdev_t *bdev;
	ni_netdev_t *pdev;
	ni_netdev_t *sdev;
	const char *type;
	...
	if (!(scfg = ni_suse_ifcfg_add_ovs_system(ifcfgs, ifcfg))) {
		ni_error("ifcfg-%s: failed to init ovs-system datapath",
				bdev->name);
		return FALSE;
	}

	file = bdev->name;
	sdev = scfg->compat->dev;
	type = ni_linktype_type_to_name(bdev->link.type);
	if (!ni_netdev_name_is_valid(port) ||
	    ni_string_eq(bdev->name, port) || ni_string_eq(sdev->name, port)) {
		ni_error("ifcfg-%s: rejecting invalid '%s' %s port name '%s'",
				bdev->name, file, type, port);
		return FALSE;
	}

	if ((pcfg = ni_suse_ifcfg_find_by_ifname(ifcfgs, port))) {
		pdev = pcfg->compat->dev;
		pconf = &pcfg->compat->port;

		if (!ni_string_empty(pdev->link.masterdev.name)) {
			/* already port of some non-ovs interface */
			if (!ni_string_eq(pdev->link.masterdev.name, sdev->name)) {
				ni_error("ifcfg-%s: cannot add %s port '%s' to '%s', already in '%s'",
						file, type, pdev->name, bdev->name,
						pdev->link.masterdev.name);
				return FALSE;
			}

			/* already port of other ovs-bridge interface */
			if (pconf->type == bdev->link.type && pconf->ovsbr &&
			    !ni_string_empty(pconf->ovsbr->bridge.name) &&
			    !ni_string_eq(pconf->ovsbr->bridge.name, bdev->name)) {
				ni_error("ifcfg-%s: cannot add %s port '%s' to '%s', already in '%s'",
						file, type, pdev->name, bdev->name,
						pconf->ovsbr->bridge.name);
				return FALSE;
			}
		}
	} else {
		pcfg = ni_suse_ifcfg_new_missing(ifcfg, port);
		if (!ni_suse_ifcfg_array_append(ifcfgs, pcfg)) {
			ni_suse_ifcfg_free(pcfg);
			ni_error("ifcfg-%s: failed to add %s port '%s' to '%s'",
					file, type, port, bdev->name);
			return FALSE;
		}
	...
	ni_suse_ifcfg_set_l2_only(pcfg, FALSE);

	ni_netdev_port_config_destroy(pconf);
	if (!ni_netdev_ref_set_ifname(&pdev->link.masterdev, sdev->name) ||
	    !ni_netdev_port_config_init(pconf, bdev->link.type) ||
	    !ni_netdev_ref_set_ifname(&pconf->ovsbr->bridge, bdev->name)) {
		ni_error("ifcfg-%s: failed to add %s port '%s' to '%s'",
			file, type, port, bdev->name);
...
__ni_suse_adjust_team_ports(ni_compat_netdev_array_t *netdevs, ni_compat_netdev_t *master)
{
	...
	for (i = 0; i < team->ports.count; i++) {
		...
		if (compat && (dev = compat->dev)) {
			...
			if (ni_netdev_port_config_init(&compat->port, NI_IFTYPE_TEAM))
				*compat->port.team = team->ports.data[i]->config;
		}
	}
}
	...
	for (i = 0; i < ovsbr->ports.count; ++i) {
		...
		if (compat) {
			if (__ni_suse_set_link_master(compat->dev, ovs_system, master->dev->name)) {
				ni_netdev_port_config_destroy(&compat->port);
				if (ni_netdev_port_config_init(&compat->port, NI_IFTYPE_OVS_BRIDGE))
					ni_netdev_ref_set_ifname(&compat->port.ovsbr->bridge, master->dev->name);
			}
		} else
...
static ni_suse_ifcfg_t *
ni_suse_ifcfg_add_ovs_system(ni_suse_ifcfg_array_t *ifcfgs, ni_suse_ifcfg_t *ifcfg)
{
	static const ni_ifworker_control_t control = {
		"hotplug", NULL, FALSE, FALSE, NI_TRISTATE_DISABLE, 0, 0
	};
	...
	if ((scfg = ni_suse_ifcfg_find_by_iftype(ifcfgs, NI_IFTYPE_OVS_SYSTEM)))
		return scfg;

	/* Consider ovs-system as a fixed/reserved datapath name for openvswitch */
	ovs_system = ni_linktype_type_to_name(NI_IFTYPE_OVS_SYSTEM);
	if ((scfg = ni_suse_ifcfg_find_by_ifname(ifcfgs, ovs_system)))
		return scfg;

	scfg = ni_suse_ifcfg_new_missing(ifcfg, ovs_system);
	if (!ni_suse_ifcfg_array_append(ifcfgs, scfg)) {
		ni_suse_ifcfg_free(scfg);
		return NULL;
	}

	scfg->compat->dev->link.type = NI_IFTYPE_OVS_SYSTEM;
	/* similar to normal hotplug, but with usercontrol=FALSE */
	if (ifcfg->compat->control)
		ni_ifworker_control_free(ifcfg->compat->control);
	ifcfg->compat->control = ni_ifworker_control_clone(&control);
	ni_suse_ifcfg_set_l2_only(scfg, FALSE);
	return scfg;
}
...
static ni_suse_ifcfg_t *
ni_suse_ifcfg_find_by_iftype(ni_suse_ifcfg_array_t *ifcfgs, ni_iftype_t iftype)
{
	ni_suse_ifcfg_t *ifcfg;
	ni_netdev_t *dev;
	unsigned int i;
