...
#define NI_REDFISH_CLIENT_CONFIG	"client-redfish.xml"
...
static int				fw_ext_config_enable(void);
static int				fw_ext_config_disable(void);
...
ni_wicked_redfish(const char *caller, int argc, char **argv)
{
	...
	while ((opt = getopt_long(argc, argv, "+h", options, NULL))  != -1) {
		...
		usage:
			fprintf(stderr,
				...
				"\n"
				"Actions:\n"
				"  enable		enable 'firmware:redfish' config extension\n"
				"  disable		disable 'firmware:redfish' config extension\n"
	...
	switch (action) {
		case ACTION_ENABLE:
			status = fw_ext_config_enable();
			goto cleanup;

		case ACTION_DISABLE:
			status = fw_ext_config_disable();
			goto cleanup;
...
static const char *
fw_ext_config_file(char **config_file)
{
	...
	if (!ni_string_printf(config_file, "%s/%s", config_dir, NI_REDFISH_CLIENT_CONFIG))
		return NULL;
...
static FILE *
fw_ext_config_open(const char *filename, char **tempname)
{
	FILE *fp;
	int fd;
...
static ni_bool_t
fw_ext_config_dump(FILE *out)
{
	int ret = fputs(
		"<!-- config file generated by `wicked redfish enable` -->\n"
		"<config>\n"
		"  <netif-firmware-discovery>\n"
		"    <script name=\"redfish\" command=\""NI_REDFISH_CONFIG_EXTENSION"\" />\n"
		"  </netif-firmware-discovery>\n"
		"</config>\n",
		out);
	return ret > 0;
...
static int
fw_ext_config_enable(void)
{
	char *config_file = NULL;
	char *config_temp = NULL;
	FILE *temp;

	if (!fw_ext_config_file(&config_file)) {
		ni_error("Unable to construct "NI_REDFISH_CONFIG_ORIGIN
				" extension config file name: %m");
		return NI_WICKED_RC_ERROR;
	}

	if (!(temp = fw_ext_config_open(config_file, &config_temp))) {
		ni_error("Unable to create '%s' config temp file: %m",
				config_file);
		ni_string_free(&config_file);
		return NI_WICKED_RC_ERROR;
	}

	if (!fw_ext_config_dump(temp)) {
		ni_error("Unable to write '%s' config temp file: %m",
				config_file);
		fclose(temp);
		unlink(config_temp);
	...
	if (rename(config_temp, config_file) < 0) {
		ni_error("Unable to create '%s' config file: %m",
			config_file);
		unlink(config_temp);
		ni_string_free(&config_temp);
		ni_string_free(&config_file);
		...
		ni_debug_application("Created '%s' config file",
				config_file);
		ni_string_free(&config_temp);
		ni_string_free(&config_file);
...
static int
fw_ext_config_disable(void)
{
	const char *config_dir = ni_get_global_config_dir();
	char *config_file = NULL;

	if (!config_dir || !ni_string_printf(&config_file, "%s/%s",
			config_dir, NI_REDFISH_CLIENT_CONFIG)) {
		ni_error("Failed to construct '%s' config file name",
				NI_REDFISH_CLIENT_CONFIG);
		return NI_WICKED_RC_ERROR;
	}

	if (unlink(config_file) < 0 && errno != ENOENT) {
		ni_error("Failed to remove config file '%s': %m", config_file);
		ni_string_free(&config_file);
		return NI_WICKED_RC_ERROR;
	} else {
		ni_string_free(&config_file);
