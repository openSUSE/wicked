...
/*
 * Build a name/type pair. The type can be complex or scalar; the definition can be longhand or
 * shorthand.
 */
#if 0
int
ni_xs_build_name_type(xml_node_t *node, ni_xs_type_dict_t *typedict, const char **namep, ni_xs_type_t **typep)
{
	const char *typeAttr;
	...
	*typep = NULL;
	...
	if (ni_xs_is_class_name(node->name)) {
		type = ni_xs_build_complex_type(node, node->name, typedict);
		if (type == NULL)
			return -1;
		*typep = type;
		return 0;
	...
	if ((typeAttr = xml_node_get_attr(node, "class")) != NULL) {
		if (ni_xs_is_reserved_name(node->name)) {
			ni_error("%s: named type node uses reserved name", xml_node_location(node));
			return -1;
		}

		if (ni_xs_typedict_lookup(typedict, node->name)) {
			ni_error("%s: ambiguous type for node <%s>", xml_node_location(node), node->name);
			return -1;
		}

		type = ni_xs_build_complex_type(node, typeAttr, typedict);
		if (type == NULL) {
			ni_error("%s: cannot build schema for node <%s> (class \"%s\") in %s", xml_node_location(node), node->name, typeAttr, __func__);
			return -1;
		}
		...
		if (namep)
			*namep = node->name;
			...
			ni_warn("%s: ignoring type name <%s>", xml_node_location(node), node->name);
	} else
	if ((typeAttr = xml_node_get_attr(node, "type")) != NULL) {
		/* This is <name type="..."/> */
		...
		/* check for
		 *  <name type="someothertype"/>
		 * where "someothertype" has been defined previously
		 */
		type = ni_xs_typedict_lookup(typedict, typeAttr);
		if (type == NULL) {
			ni_error("%s: node <%s> of unknown type \"%s\"", xml_node_location(node), node->name, typeAttr);
			return -1;
		}
		...
		if (namep)
			*namep = node->name;
			...
			ni_warn("%s: ignoring type name <%s>", xml_node_location(node), node->name);
		ni_xs_type_hold(type);
	} else {
		/* Check for anonymous struct member
		 *  <footype/>
		 */
		if (!(type = ni_xs_typedict_lookup(typedict, node->name))) {
			ni_error("%s: unknown type <%s>", xml_node_location(node), node->name);
			return -1;
		}
		ni_xs_type_hold(type);
	}
