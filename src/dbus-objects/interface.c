 ...
 */
static dbus_bool_t
ni_objectmodel_netif_set_ifstate(ni_dbus_object_t *object, const ni_dbus_method_t *method,
			unsigned int argc, const ni_dbus_variant_t *argv,
			ni_dbus_message_t *reply, DBusError *error)
{
	ni_netdev_t *dev;
	...
	ifstate = ni_ifstate_new(0);
	if (!ni_objectmodel_netif_ifstate_from_dict(ifstate, &argv[0])) {
...
static ni_dbus_method_t		ni_objectmodel_netif_methods[] = {
	...
	{ "setIfState",		"a{sv}",		ni_objectmodel_netif_set_ifstate },
...
/*
 * Property Interface.ifstate
 */
static dbus_bool_t
__ni_objectmodel_netif_get_ifstate(const ni_dbus_object_t *object,
				const ni_dbus_property_t *property,
				ni_dbus_variant_t *result,
				DBusError *error)
{
	...
	if ((ifstate = dev->ifstate) == NULL)
		return ni_dbus_error_property_not_present(error, object->path, property->name);

	ni_dbus_variant_init_dict(result);
	return ni_objectmodel_netif_ifstate_to_dict(ifstate, result);
}

dbus_bool_t
ni_objectmodel_netif_ifstate_to_dict(const ni_ifstate_t *ifstate, ni_dbus_variant_t *dict)
{
	char *value = NULL;

	ni_dbus_dict_add_bool(dict, "persistent", (dbus_bool_t) ifstate->persistent);
	ni_dbus_dict_add_uint32(dict, "init-state", ifstate->init_state);

	ni_dbus_dict_add_string(dict, "init-time",
		ni_ifstate_print_timeval(&ifstate->init_time, &value));
	ni_string_free(&value);

	ni_dbus_dict_add_string(dict, "last-time",
		ni_ifstate_print_timeval(&ifstate->last_time, &value));
	ni_string_free(&value);
...
static dbus_bool_t
__ni_objectmodel_netif_set_ifstate(ni_dbus_object_t *object,
				const ni_dbus_property_t *property,
				const ni_dbus_variant_t *argument,
				DBusError *error)
{
	...
	ifstate = ni_ifstate_new(0);
	if (!ni_objectmodel_netif_ifstate_from_dict(ifstate, argument)) {
		ni_ifstate_free(ifstate);
		return FALSE;
	}
	ni_netdev_set_ifstate(dev, ifstate);
...
dbus_bool_t
ni_objectmodel_netif_ifstate_from_dict(ni_ifstate_t *ifstate, const ni_dbus_variant_t *dict)
{
	const char *sval;

	ni_dbus_dict_get_bool(dict, "persistent", (dbus_bool_t *) &ifstate->persistent);
	ni_dbus_dict_get_uint32(dict, "init-state", &ifstate->init_state);

	if (ni_dbus_dict_get_string(dict, "init-time", &sval)) {
		if (!ni_ifstate_parse_timeval(sval, &ifstate->init_time))
			return FALSE;
	}

	if (ni_dbus_dict_get_string(dict, "last-time", &sval)) {
		if (!ni_ifstate_parse_timeval(sval, &ifstate->last_time))
			return FALSE;
	}
