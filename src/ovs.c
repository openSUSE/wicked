...
ni_ovs_vsctl_bridge_to_vlan(const char *brname, uint16_t *vlan)
{
	...
	ni_process_free(pi);
	if (rv) {
		ni_debug_application("%s: unable to query bridge vlan", brname);
		goto failure;
	}
...
ni_ovs_vsctl_bridge_to_parent(const char *brname, char **parent)
{
	...
	ni_process_free(pi);
	if (rv) {
		ni_debug_application("%s: unable to query bridge parent", brname);
		goto failure;
	}
...
ni_ovs_vsctl_bridge_ports(const char *brname, ni_netdev_ref_array_t *ports)
{
	...
	ni_process_free(pi);
	if (rv) {
		ni_debug_application("%s: unable to query bridge ports", brname);
		goto failure;
	}
...
ni_ovs_vsctl_bridge_port_to_bridge(const char *pname, char **brname)
{
	...
	if (rv != NI_PROCESS_SUCCESS) {
		ni_debug_application("%s: unable to query port bridge", pname);
		goto failure;
	}
