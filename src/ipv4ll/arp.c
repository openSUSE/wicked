/*
 * ARP send and receive routines for IPv4LL
 *
...
static int	ni_arp_send(ni_autoip_device_t *dev, unsigned int arpop,
 ...
 */
int
ni_arp_socket_open(ni_autoip_device_t *dev)
{
	...
	dev->capture = ni_capture_open(&dev->devinfo, ETHERTYPE_ARP, ni_arp_socket_recv);
	if (!dev->capture)
		return -1;
...
ni_arp_socket_recv(ni_socket_t *sock)
{
	...
	if (ni_capture_recv(capture, &buf) >= 0) {
		ni_autoip_device_t *dev = ni_capture_get_user_data(capture);

		ni_autoip_fsm_process_arp_packet(dev, &buf);
	}
}

int
ni_arp_send_request(ni_autoip_device_t *dev, struct in_addr sip,
			const ni_hwaddr_t *tha, struct in_addr tip)
{
	return ni_arp_send(dev, ARPOP_REQUEST, &dev->devinfo.hwaddr, sip, tha, tip);
}

int
ni_arp_send(ni_autoip_device_t *dev, unsigned int arpop,
		const ni_hwaddr_t *sha, struct in_addr sip,
		const ni_hwaddr_t *tha, struct in_addr tip)
{
	...
	hwlen = ni_link_address_length(dev->devinfo.iftype);
	...
	arp->ar_hrd = htons(dev->devinfo.arp_type);
...
int
ni_arp_parse_reply(ni_autoip_device_t *dev, ni_buffer_t *bp, struct in_addr *sip, ni_hwaddr_t *sha)
{
	struct arphdr *arp;
	...
	sha->type = dev->devinfo.iftype;
	sha->len = ni_link_address_length(dev->devinfo.iftype);
