...
ni_arp_send(ni_arp_socket_t *arph, const ni_arp_packet_t *packet)
{
	...
	hwlen = ni_link_address_length(arph->dev_info.hwaddr.arp_type);
	...
	arp->ar_hrd = htons(arph->dev_info.hwaddr.arp_type);
...
ni_arp_parse(ni_arp_socket_t *arph, ni_buffer_t *bp, ni_arp_packet_t *p)
{
	...
	p->op = arp->ar_op;
	p->sha.arp_type = arph->dev_info.hwaddr.arp_type;
	p->sha.len = ni_link_address_length(arph->dev_info.hwaddr.arp_type);
	p->tha = p->sha;
