...
int
ni_call_set_ifstate(ni_dbus_object_t *object, const ni_ifstate_t *ifstate)
{
	const ni_dbus_service_t *service;
	const ni_dbus_method_t *method;
	ni_dbus_variant_t dict;
	...
	if ((rv = ni_get_device_method(object, "setIfState", &service, &method)) < 0)
		return rv;

	memset(&dict, 0, sizeof(dict));
	ni_dbus_variant_init_dict(&dict);
	if (!ni_objectmodel_netif_ifstate_to_dict(ifstate, &dict))
		return -1;
