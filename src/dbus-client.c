...
dbus.SystemBus
	really BusConnection(TYPE_SYSTEM)
	calls _new_for_bus

	->
	conn = dbus_bus_get_private(DBUS_BUS_SYSTEM, &error);
	libdbusconn = DBusPyLibDBusConnection_New (conn); /* this is just a container */
	dbus_connection_unref (conn);

	self = (Connection *)(DBusPyConnection_Type.tp_new)(cls, ...)
		/* this one encapsulates connection objects w/ methods */

	get_object -> returns a proxy object (bus interface name and object path)
	object.Interface -> Returns a MethodProxy (bus interface + object path + method name)

	Calling a MethodProxy member function:
	...
	This boils down to dbus_connection_send_with_reply()
